...
use vibetunnel_pty::core::{PtyConfig, SessionInfo, SessionStore};
use vibetunnel_pty::core::pty::{create_pty, resize_pty};
use vibetunnel_pty::core::PtyHandle;
use super::{
...
impl Forwarder {
    ...
    pub async fn run(&mut self, command: Vec<String>) -> Result<()> {
        ...
        // Forward I/O
        let result = self
            .forward_io(&mut handle, socket_client, shutdown)
            .await;
    ...
    async fn forward_io(
        ...
        // Take ownership and store in Arc<Mutex>
        {
            ...
            *w = Some(std::mem::replace(&mut handle.writer, Box::new(std::io::sink())));
            
            let mut r = reader.lock().await;
            *r = Some(std::mem::replace(&mut handle.reader, Box::new(std::io::empty())));
            
            let mut m = master.lock().await;
            *m = Some(std::mem::replace(&mut handle.master, Box::new(DummyMaster)));
        }
